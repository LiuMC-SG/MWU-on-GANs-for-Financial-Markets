"""
Unified CNN‑LSTM‑GAN training & Optuna optimization runner.

This module merges the functionality of:
  - `cnn_lstm_gan_runner.py`  (plain training per ticker)
  - `cnn_lstm_gan_optuna_optimizer.py` (Optuna hyperparameter search)
...
--------
Train:
    python -m cnn_lstm_gan_train_optuna train \
        --tickers AAPL,MSFT \
        --data-dir ~/Data/OHLC \
        --out-dir ./runs

Optimize (and then train the best model):
    python -m cnn_lstm_gan_train_optuna optimize \
        --ticker AAPL \
        --data-dir ~/Data/OHLC \
...
# --- Model dependency ---
from cnn_lstm_seq_ohlcav.cnn_lstm_gan_model_sequential import LSTMCNNGANExpert

# --- Optuna base optimizer ---
from common.optuna_common import BaseOptunaOptimizer  # uses the "common.*" shims above

LOG = logging.getLogger("cnn_lstm_gan_train_optuna")
