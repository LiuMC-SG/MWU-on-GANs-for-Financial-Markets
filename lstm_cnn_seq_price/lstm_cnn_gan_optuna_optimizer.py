...
from cnn_lstm_gan_model_sequential import LSTMCNNGANExpert
from lstm_cnn_gan_runner import (
...
class GANOptunaOptimizer:
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            model = LSTMCNNGANExpert(
                ...
                cnn_out_channels_G=[64, 128, 256, 512],  # Keep architecture fixed
                cnn_out_channels_D=[64, 128, 256, 512],
            )
            ...
            LOG.info(f"  lr_anomaly={learning_rate_anomaly:.2e}, resample_z={resample_z}")
            LOG.info(f"  lstm_hidden_size_G={lstm_hidden_size_g}, lstm_num_layers_G={lstm_num_layers_g}, dropout_G={dropout_g}")
            LOG.info(f"  lstm_hidden_size_D={lstm_hidden_size_d}, lstm_num_layers_D={lstm_num_layers_d}, dropout_D={dropout_d}")
    ...
    def train_best_model(self) -> LSTMCNNGANExpert:
        ...
        best_model = LSTMCNNGANExpert(
            ...
            lstm_hidden_size_G=self.best_params['lstm_hidden_size_g'],
            lstm_num_layers_G=self.best_params['lstm_num_layers_g'],
            dropout_G=self.best_params['dropout_g'],
            lstm_hidden_size_D=self.best_params['lstm_hidden_size_d'],
            lstm_num_layers_D=self.best_params['lstm_num_layers_d'],
            dropout_D=self.best_params['dropout_d'],
            lambda_anom=0.9,
            cnn_out_channels_G=[64, 128, 256, 512],
            cnn_out_channels_D=[64, 128, 256, 512],
        )
