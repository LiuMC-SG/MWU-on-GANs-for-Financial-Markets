...
class Generator(nn.Module):
    def __init__(
        ...
        # Initial projection for LSTM input
        self.lstm_projection = nn.Linear(latent_dim, lstm_hidden_size)
        
        # Stage 1: LSTM - Sequential generation
        self.lstm = nn.LSTM(
            lstm_hidden_size, 
            lstm_hidden_size, 
            lstm_num_layers, 
        ...
        # Projection from LSTM output to CNN input channels
        self.lstm_to_cnn = nn.Linear(lstm_hidden_size, cnn_out_channels[0])
        
        # Stage 2: CNN - Feature refinement
        cnn_layers = []
        in_channels = cnn_out_channels[0]
        for i, out_channels in enumerate(cnn_out_channels[1:], 1):
            cnn_layers.extend([
        ...
        # Stage 3: Final output layers
        self.final_layers = nn.Sequential(
            nn.Conv1d(cnn_out_channels[-1], cnn_out_channels[-1] // 2, kernel_size=3, padding=1),
            nn.BatchNorm1d(cnn_out_channels[-1] // 2),
            nn.LeakyReLU(leaky_relu_slope),
            nn.Dropout1d(dropout * 0.5),
            nn.Conv1d(cnn_out_channels[-1] // 2, features, kernel_size=3, padding=1),
        )
    ...
    def forward(self, z):
        ...
        # Stage 1: LSTM sequential generation
        # Create sequence input from latent vector
        lstm_input = self.lstm_projection(z).unsqueeze(1)  # (batch, 1, lstm_hidden_size)
        lstm_input = lstm_input.repeat(1, self.sequence_length, 1)  # (batch, seq_len, lstm_hidden_size)
        
        lstm_out, _ = self.lstm(lstm_input)  # (batch, seq_len, lstm_hidden_size)
        
        # Stage 2: Prepare for CNN processing
        # Project LSTM output to CNN input channels
        lstm_out_reshaped = lstm_out.contiguous().view(-1, self.lstm_hidden_size)  # (batch*seq_len, lstm_hidden_size)
        cnn_input = self.lstm_to_cnn(lstm_out_reshaped)  # (batch*seq_len, cnn_out_channels[0])
        cnn_input = cnn_input.view(batch_size, self.sequence_length, -1)  # (batch, seq_len, cnn_out_channels[0])
        cnn_input = cnn_input.transpose(1, 2)  # (batch, cnn_out_channels[0], seq_len)
        
        # Stage 2: CNN feature refinement
        x = self.cnn_seq(cnn_input)  # (batch, cnn_out_channels[-1], seq_len)
        
        # Stage 3: Final output generation
        output = self.final_layers(x)  # (batch, features, seq_len)
        output = self.final_activation(output)
        
        return output

# -----------------------------
# Discriminator (Sequential LSTM → CNN)
# -----------------------------
class Discriminator(nn.Module):
    def __init__(
        ...
        # Stage 1: LSTM - Sequential analysis
        self.lstm = nn.LSTM(
        ...
        # Projection from LSTM output to CNN input
        lstm_output_size = lstm_hidden_size * 2  # Bidirectional
        self.lstm_to_cnn = nn.Linear(lstm_output_size, cnn_out_channels[0])
        
        # Stage 2: CNN - Feature refinement and spatial analysis
        cnn_layers = []
        in_channels = cnn_out_channels[0]
        for out_channels in cnn_out_channels[1:]:
            cnn_layers.extend([
                nn.Conv1d(in_channels, out_channels, kernel_size=3, padding=1),
                nn.BatchNorm1d(out_channels),
                nn.LeakyReLU(leaky_relu_slope),
        ...
        # Stage 3: Final classification layers
        self.classifier = nn.Sequential(
            nn.Linear(cnn_out_channels[-1], cnn_out_channels[-1] // 2),
            nn.LeakyReLU(leaky_relu_slope),
            nn.Dropout(dropout),
            nn.Linear(cnn_out_channels[-1] // 2, cnn_out_channels[-1] // 4),
            nn.LeakyReLU(leaky_relu_slope),
            nn.Dropout(dropout),
            nn.Linear(cnn_out_channels[-1] // 4, 1),
        )
    ...
    def forward(self, x):
        ...
        lstm_input = x.transpose(1, 2)  # (batch, seq_len, features)
        lstm_out, _ = self.lstm(lstm_input)  # (batch, seq_len, lstm_hidden_size * 2)
        
        # Stage 2: Prepare for CNN processing
        # Project LSTM output to CNN input channels
        lstm_out_reshaped = lstm_out.contiguous().view(-1, self.lstm_hidden_size * 2)  # (batch*seq_len, lstm_hidden_size * 2)
        cnn_input = self.lstm_to_cnn(lstm_out_reshaped)  # (batch*seq_len, cnn_out_channels[0])
        cnn_input = cnn_input.view(batch_size, self.sequence_length, -1)  # (batch, seq_len, cnn_out_channels[0])
        cnn_input = cnn_input.transpose(1, 2)  # (batch, cnn_out_channels[0], seq_len)
        ...
        x = self.global_pool(x).squeeze(-1)  # (batch, cnn_out_channels[-1])
