...
class AnomalyDetector:
    ...
    def _initialize_model(self):
        ...
        try:
            if GANExpert is None:
                self.logger.warning("GANExpert not available, using mock anomaly detection")
                return
                
            self.model = GANExpert(seq_len=self.lookback_window)
    ...
    def compute_daily_anomaly_scores(self, prices: pd.DataFrame) -> pd.Series:
        ...
        if self.start is not None:
            ...
            # start_bound = pd.Timestamp(self.start).normalize() - pd.Timedelta(days=int(self.lookback_window*4))
    ...
    def _predict_anomaly_batch(self, windows: np.ndarray) -> np.ndarray:
        ...
        """
        if self.model is None or GANExpert is None:
